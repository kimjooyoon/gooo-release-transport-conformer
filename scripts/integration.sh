...
jq -e '.activities | length == 8 and (unique | length) == 8' "$work/generated/transport-manifest.json" >/dev/null
jq -s -e 'length == 8 and (map(.activity) | unique | length == 8) and (map(.ordinal) == [1,2,3,4,5,6,7,8])' < <(sed '/^[[:space:]]*$/d' "$work/generated/transport-events.ndjson") >/dev/null
