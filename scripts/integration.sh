...
jq -e '.activities | length == 9 and (unique | length) == 9' "$work/generated/transport-manifest.json" >/dev/null
jq -s -e 'length == 9 and (map(.activity) | unique | length == 9) and (map(.ordinal) == [1,2,3,4,5,6,7,8,9])' < <(sed '/^[[:space:]]*$/d' "$work/generated/transport-events.ndjson") >/dev/null
