...
The fixed denominator has exactly twelve scenarios: four closed transport
contracts, three explicit unknowns, and five preserved refutations. Scenario
...
12-scenario result is separate, and from the PR onward GitHub Actions is the
...
annotated tag, creates a draft release before uploading assets, uploads every
asset, publishes only after upload, and verifies the public release, tag
object, and asset digests. It does not call an administration endpoint or
consume a user token secret. The operator immutable-release setting is enabled
