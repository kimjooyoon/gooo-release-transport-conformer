    ...
    steps:
      ...
      - name: Create annotated tag exactly once
        ...
        run: |
          set -Eeuo pipefail
          release_list=$(gh api "repos/$GITHUB_REPOSITORY/releases?per_page=100")
          exact_draft_count=$(jq --arg tag "$RELEASE_TAG" --arg expected "$EXPECTED_SHA" '[.[] | select(.tag_name == $tag and .draft == true and .target_commitish == $expected)] | length' <<<"$release_list")
          if [[ "$exact_draft_count" = 1 ]]; then
            test "$(git ls-remote --exit-code --tags origin "refs/tags/$RELEASE_TAG" | wc -l | tr -d ' ')" = 1
          elif [[ "$exact_draft_count" = 0 ]]; then
            if git ls-remote --exit-code --tags origin "refs/tags/$RELEASE_TAG" >/dev/null 2>&1; then
              echo "refusing to reuse existing tag without an exact draft" >&2
              exit 1
            fi
            ...
            echo "refusing ambiguous exact draft set for $RELEASE_TAG" >&2
            exit 1
          fi
      ...
      - name: Create draft release before assets
        ...
        run: |
          ...
          if [[ "$draft_count" = 0 ]]; then
            gh release create "$RELEASE_TAG" --target "$EXPECTED_SHA" --verify-tag --draft --title "gooo release transport $RELEASE_TAG" \
              --notes "Draft release; exact list-ID reconciliation must pass before publication."
      ...
      - name: Reconcile existing draft by list ID
        ...
        run: |
          set -Eeuo pipefail
          release_list=$(gh api "repos/$GITHUB_REPOSITORY/releases?per_page=100")
          tag_drafts=$(jq --arg tag "$RELEASE_TAG" --arg expected "$EXPECTED_SHA" '[.[] | select(.tag_name == $tag and .draft == true) | {id,tag_name,draft,target_commitish}]' <<<"$release_list")
          test "$(jq 'length' <<<"$tag_drafts")" = 1
          jq -e --arg expected "$EXPECTED_SHA" '.[0].tag_name != "" and .[0].draft == true and .[0].target_commitish == $expected' <<<"$tag_drafts" >/dev/null
        ...
        run: |
          ...
          if [[ "$existing_count" = 0 ]]; then
            for asset in "$source_name" "$evidence_name" "$checksum_name"; do
              gh api --method POST -H 'Content-Type: application/octet-stream' --input "$RELEASE_DIR/$asset" "repos/$GITHUB_REPOSITORY/releases/$DRAFT_ID/assets?name=$asset" >/dev/null
            done
      ...
      - name: Publish release after all uploads
        ...
        run: |
          ...
          gh api --method PATCH -F draft=false "repos/$GITHUB_REPOSITORY/releases/$DRAFT_ID" >/dev/null
